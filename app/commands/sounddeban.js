...
module.exports = function(bot, data, params) {
    ...
    connection.query('SELECT * FROM perm WHERE user_id = ? AND rank = "op"', [data.userID], function (err1, rows1, a) {
        ...
        if(rows1.length == 1) {
            connection.query('SELECT * FROM perm WHERE user_id = ? AND rank = "ban"', [data.rawEvent.d.mentions[0].id], function (err2, rows2, b) {
                ...
                if (rows2.length == 1) {
                    ...
                    errors.push("Cette personne n'est pas banni");
                }
            });
        } else {
