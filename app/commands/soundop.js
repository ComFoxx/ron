...
module.exports = function(bot, data, params) {
    ...
    connection.query('SELECT * FROM perm WHERE user_id = ? AND rank = "op"', [data.userID], function (err1, rows1, a) {
        ...
        if(rows1.length === 1) {
            connection.query('SELECT * FROM perm WHERE user_id = ?', [data.rawEvent.d.mentions[0].id], function (err2, rows2, b) {
                if (err2) console.error(err2);
                if (rows2.length === 0) {
                    connection.query('INSERT INTO perm (user_id, rank) VALUES (?, "op")', [data.rawEvent.d.mentions[0].id], function (err3, rows3, c) {
                        if (err3) console.error(err3);
                    });
                } else {
                    if (rows2[0].rank === "ban") {
                        errors.push("Cette personne est banni !");
                    } else if (rows2[0].rank == "op") {
                        errors.push("Cette personne est déjà opératrice !");
                    } else {
